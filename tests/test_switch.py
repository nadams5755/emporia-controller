...
def make_switch(mode=ChargeMode.STOPPED, label="Stop Charging", evse_name="My EVSE", current_mode=None):
...
def test_switch_name_is_evse_name_and_label():
    sw = make_switch(label="Stop Charging", evse_name="My EVSE")
    assert sw._attr_name == "My EVSE - Stop Charging"

def test_switch_unique_id_uses_domain_slug_and_mode():
    sw = make_switch(mode=ChargeMode.STOPPED)
    assert sw._attr_unique_id == f"{DOMAIN}_switch_evse1_stopped"
...
def test_is_on_when_mode_matches():
    sw = make_switch(mode=ChargeMode.STOPPED, current_mode=ChargeMode.STOPPED)
    assert sw.is_on is True

def test_is_off_when_mode_does_not_match():
    sw = make_switch(mode=ChargeMode.STOPPED, current_mode=ChargeMode.EXCESS_SOLAR)
...
async def test_turn_on_stopped():
    sw = make_switch(mode=ChargeMode.STOPPED)
    await sw.async_turn_on()
    sw.coordinator.set_mode.assert_called_once_with("switch.evse1", ChargeMode.STOPPED)
